...
func _physics_process(_delta: float) -> void:
	...
	for i: int in player.get_slide_collision_count():
		...
		var right: float = level_bounds.position.x + level_bounds.size.x
		var top: float = level_bounds.position.y
		var bottom: float = level_bounds.position.y + level_bounds.size.y
